...
#from selenium.webdriver.support.ui import WebDriverWait #pour faire des temps d'attentes de chargement de page
from selenium.webdriver.support.ui import Select #pour gérer les différents tabs
#from selenium.webdriver.support import expected_conditions as EC
#from selenium.webdriver.chrome.options import Options pour les option
...
# Variables initiales
app_name = "ADP_demo"
workspace = "D:/workspace/python/"+app_name+"/"
download_folder = "C:\\Apps\\"+app_name
with open(workspace+'credentials.txt') as f:
    myCred = []
    for ligne in f:
        myCred.append(ligne.strip())
...
if len(myFile) > 0:
    ...
    try:
        ...
        #input user + pwd
        driver.find_elements(By.ID, "login")[0].send_keys(username)
        driver.find_elements(By.ID, "login-pw")[0].send_keys(pwd)
        #wait then log in
        driver.implicitly_wait(5)
        driver.find_element(By.XPATH, "/html/body/div/div/div/div[1]/div[3]/div[4]/div/form/div[4]/button").click()
        #wait then open Activité page in new tab
        time.sleep(2)
        driver.find_element(By.XPATH, '//*[@id="menu_2_navItem"]').click()
        driver.implicitly_wait(5)
        driver.find_element(By.XPATH, '//*[@id="menu_2_ttd_0_0"]/span[2]').click()
        time.sleep(2)
        #switch to old tab
        ...
        for date in myFile:
            ...
            driver.implicitly_wait(5)
            driver.find_element(By.XPATH, '//*[@id="btn_acc_add_label"]').click()
            time.sleep(1)
            #select event family (Télétravail)
            select_TT = Select(driver.find_element(By.ID, 'evt_famille'))
            select_TT.select_by_visible_text('Télétravail')
            time.sleep(2)
            #select event nature (Télétravail)
            select_EVT = Select(driver.find_element(By.ID, 'evt_natSelect'))
            select_EVT.select_by_visible_text('Télétravail')
            #go on datepicker
            datepicker = driver.find_elements(By.ID, "evt_datedeb")[0]
            ...
            time.sleep(1)
            #Save button
            driver.find_element(By.ID, 'evt_save_label').click()
            time.sleep(1)
            #Get box title value in variable to testing if error 
            titleBox = driver.find_element(By.ID, 'dialogBoxGen_1_title')
            #close title box button
            driver.find_element(By.ID, 'btn1_label').click()
            #test title value
            if titleBox.text == 'Erreur':
                logger.error('Problème avec la date: '+date)
                #close saisir evenement for the following date
                driver.find_element(By.ID, 'evt_close_label').click()
            #if no error
            else:
